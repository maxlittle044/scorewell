        ...
        <div className="flex flex-col gap-12">
          {bank.groups.map((group) => (
            <section key={group.theme}>
              <h2 className="text-lg font-bold text-ink">{group.theme}</h2>
              {group.note && (
                <p className="mb-5 mt-1.5 text-sm leading-relaxed text-ink-body">{group.note}</p>
              )}
                ...
                {group.questions.map((question, index) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      {question.label && (
                        <span className="mt-2 inline-block rounded-full bg-brand-50 px-2.5 py-0.5 text-xs font-medium text-link">
                          {question.label}
                        </span>
                      )}
                    </div>
