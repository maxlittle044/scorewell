  ...
  return (
    <div>
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {SKILLS.map((label, i) => (
          <div key={label}>
            <label htmlFor={`skill-${i}`} className="mb-1.5 block text-sm font-medium text-ink-body">
              {label}
            </label>
            <select
              id={`skill-${i}`}
      ...
      <div className="mt-6 rounded-xl bg-brand-50 px-5 py-4 text-center">
        <p className="text-xs font-medium uppercase tracking-wide text-link">Overall band</p>
        <p className="mt-1 text-3xl font-bold text-link">{overall}</p>
      </div>
    </div>
  );
}
