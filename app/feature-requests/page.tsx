...
function StatusBadge({ status }: { status: Request["status"] }) {
  const styles: Record<Request["status"], string> = {
    ...
    "Under review": "bg-zinc-100 text-zinc-600",
  };
  return (
  ...
  return (
    <main className="flex flex-1 flex-col bg-white">
      <div className="mx-auto w-full max-w-2xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <ul className="flex flex-col divide-y divide-zinc-200 rounded-2xl border border-zinc-200">
          {requests.map((request) => (
            <li key={request.id} className="flex items-center gap-4 px-5 py-4">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-zinc-900">{request.title}</p>
              </div>
        ...
        <form className="mt-6 flex flex-col gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 p-5">
          ...
          <textarea
            ...
            className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-100"
          />
          <button
            type="submit"
